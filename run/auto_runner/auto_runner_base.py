...
class AutoRunnerBase(ABC):
    def __init__(self, env_config_path,
        ...
        self.result_analyzer = ExperimentAnalyzer(experiment_path=self.experiment_path,
                                                  success_conditions=self.success_conditions)
        self.tolerance = tolerance
        self.sim_env = EnvironmentManager(env, max_speed=max_speed)
        self.code_runner = CodeRunner(time_out=experiment_duration,
    ...
    def get_experiment_directories(self):
        ...
        if self.run_mode == 'rerun' or self.run_mode == 'fail_rerun':
            ...
            directories = directories[(batch_num - 1) * batch_size:batch_num * batch_size]
            print(f"Batch {batch_num} from {batch_size * (batch_num - 1)} to {batch_size * batch_num}")
        return directories
                ...
                for experiment in experiment_list:
                    ...
                    while retries < max_retries and not success:
                        if self.script_name == 'run_meta.py':
                            setup_metagpt(os.path.join(self.experiment_path, experiment))
                        ...
                        wo_vlm_result, vlm_result = self.code_runner.load_result(experiment)
                        if wo_vlm_result is not None:
                            wo_vlm_analysis = self.analyze_result(wo_vlm_result)
                            wo_experiment_success = self.result_analyzer.calculate_success(wo_vlm_analysis)
                        ...
                        # 检查是否有未移动的机器人
                        unmoved_robots = check_robots_no_movement_in_first_third(wo_vlm_result)
                    ...
                    print(f"Analysis for wo_experiment {experiment}: {wo_vlm_analysis},\n"
                          )

                    self.save_experiment_result(
                        os.path.join(self.experiment_path, experiment),
                        wo_vlm_result, wo_vlm_analysis, file_name='wo_vlm.json')
                    # self.save_experiment_result(
                    #     os.path.join(self.experiment_path, experiment),
