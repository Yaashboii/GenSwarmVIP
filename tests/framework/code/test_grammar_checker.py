...
class TestGrammarChecker(unittest.TestCase):
    ...
    def test_find_function_name_from_error(self):
        function_name, error_code_line = self.grammar_checker._find_function_name_from_error(os.path.join(root_manager.project_root, self.file_path), 3)
    ...
    def test_check_code_errors(self, mock_logger):        
        errors = self.grammar_checker.check_code_errors(os.path.join(root_manager.project_root, self.file_path))
        self.assertEqual(errors, self.errors)
