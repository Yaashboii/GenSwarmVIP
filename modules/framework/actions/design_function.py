...
class DesignFunction(ActionNode):
    ...
    def _build_prompt(self):
        self._context.log.format_message(f"Function: {self._function.name}", "warning")
        constraint_text = ''
...
if __name__ == "__main__":
    ...
    design_functions._context.load_from_file(path + "/analyze_function_stage.pkl")
    asyncio.run(design_functions.run())

    design_functions._context.save_to_file(f'{path}/design_function_stage.pkl')

    # import pickle

    # with open('/src/tests/design_function_stage.pkl', 'rb') as f:
    #     data = pickle.load(f)
