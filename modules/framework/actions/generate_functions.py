...
class GenerateFunctions(ActionNode):
    def __init__(self, next_text: str = "", node_name: str = "", run_mode="layer"):
        ...
        self._cost_time = 0
        self.skill_tree = self.context.global_skill_tree if self.context.scoop == "global" else self.context.local_skill_tree
        ...
        debug_error = DebugError(self.skill_tree, )
    ...
    async def _run(self) -> str:
        ...
        while not finish:
            ...
            finish = all(
                node.state == State.CHECKED for node in self.skill_tree.nodes
            )
        ...
        logger.log(f"Generate {self.context.scoop} functions cost time: {self._cost_time}", "warning")
