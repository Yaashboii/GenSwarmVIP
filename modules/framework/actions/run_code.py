    ...
    async def _run(self):
        try:
            ...
            if len(self.context.global_skill_tree.layers) == 0:
                ...
                self.env.start_environment(experiment_path=self.context.args.experiment_path)
                result = await run_script(
                    working_directory=root_manager.workspace_root, command=command, timeout=10
                )
        finally:
    ...
    def _process_response(self, result: str):
        ...
        if result == "No task to run":
            ...
            if self.call_times >= 3:
                logger.log(content=f"Run code failed {self.call_times} times, retrying...", level="warning")
                self._next = None
                return 'Fail'
            result_content = result[0]

            if self.context.args.run_mode in ['debug', 'full_version']:
                return Bug(error_msg=result_content,
...
class RunCodeAsync(ActionNode):
    ...
    def __init__(self, next_text: str = "", node_name: str = "", env: EnvironmentManager = None):
        self.call_times = 0
        self.env = env
        super().__init__(next_text, node_name)
    ...
    async def _run(self):
        ...
        try:
            if len(self.context.global_skill_tree.layers) == 0:
                ...
                keep_entities = True
            self.env.start_environment(experiment_path=root_manager.workspace_root, keep_entities=keep_entities)
    ...
    def _process_response(self, result: list):
        ...
        if self.context.args.run_mode in ['debug', 'full_version']:
            return Bug(error_msg=result_content, error_code='\n\n'.join(self.context.local_skill_tree.functions_body),
                       error_function='')
        else:
            return 'Fail'
        # else:
...
def runcode(timeout=20, feedback="None", experiment_path="clustering/2024-10-21_03-04-33",
            target_pkl="WriteRun.pkl", script="run.py", human_feedback=False, env_manager=None, debug=False):
    """
    ...
    parser.add_argument("--feedback", type=str, default="None", help="Optional: human, VLM, None, Result feedback")
    ...
    parser.add_argument("--run_mode", type=str, default='wo_vlm', help="Optional: wo_vlm,full_version,debug,vlm ")
