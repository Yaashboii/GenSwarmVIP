...
class CodingStage(Stage):
    ...
    async def _design_functions(self, async_mode=False):
        if async_mode:
            tasks = []
            for i, function in self._context.function_pool.functions.items():
                ...
                function_list = [f.text for f in self._context.function_pool.functions.values() if
            ...
            for i, function in self._context.function_pool.functions.items():
                ...
                function_list = [f.content for f in self._context.function_pool.functions.values() if
                                 f.name != function.name and f.content]
                prompt = DesignFunction_PROMPT_TEMPLATE.format(
                    task_des=TASK_DES,
                    robot_api=ROBOT_API,
                    env_des=ENV_DES,
                ...
                await self._design_function(prompt)

    async def _write_function(self, prompt: str):
        self._action = WriteCode()
        await self._action.run(prompt=prompt)

    async def _write_functions(self, async_mode=False):
        if async_mode:
            tasks = []
            for i, function in self._context.function_pool.functions.items():
                constraint_text = ''
    ...
    async def _run(self) -> StageResult:
        # await self._design_functions(async_mode=False)
        await self._write_functions(async_mode=True)
        return StageResult()
