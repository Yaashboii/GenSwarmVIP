...
class PyBullet2DEngine(Engine):
    def __init__(self, world_size: tuple | list | np.ndarray,
        ...
        p.setAdditionalSearchPath(pybullet_data.getDataPath())  # Set PyBullet's data path
    ...
    def add_entity(self, entity: Entity):
        ...
        if isinstance(entity.size, float):  # Circle
            collision_shape = p.createCollisionShape(p.GEOM_CYLINDER, radius=entity.size, height=0.01)
        else:  # Rectangle
            collision_shape = p.createCollisionShape(p.GEOM_BOX,
                                                     halfExtents=[entity.size[0] / 2, entity.size[1] / 2, 0.01])
        ...
        if self._collision_check:
            body_id = p.createMultiBody(mass, collision_shape, -1,
            ...
            body_id = p.createMultiBody(mass, collision_shape, -1,
                                        basePosition=[entity.position[0], entity.position[1], 0])
            p.setCollisionFilterGroupMask(body_id, -1, 0, 0)  # Disable collision for this body
    ...
    def apply_force(self, entity_id: int, force: np.ndarray):
        ...
        if entity_id in self.entity_map:
            body_id = self.entity_map[entity_id]["body_id"]
            p.applyExternalForce(body_id, -1, [force[0], force[1], 0], [0, 0, 0], p.WORLD_FRAME)
        else:
            raise ValueError(f"Entity {entity_id} does not exist in the environment.")
    ...
    def control_velocity(self, entity_id: int, desired_velocity: np.ndarray, dt=None):
        ...
        if entity_id in self.entity_map:
            body_id = self.entity_map[entity_id]["body_id"]
            current_velocity = np.array(p.getBaseVelocity(body_id)[0][:2])  # Get current 2D velocity
            ...
            if mass > 0:  # Only apply force to moveable objects
                ...
                force = mass * velocity_diff / (dt if dt else 1 / 10)  # Assuming 10 Hz time step
    ...
    def add_joint(self, entity_id1: int, entity_id2: int, distance: float):
        ...
        if self._joint_constraint:
            if entity_id1 in self.entity_map and entity_id2 in self.entity_map:
                ...
                self._entities[entity_id1].position = np.array(p.getBasePositionAndOrientation(body_id1)[0][:2])
                self._entities[entity_id2].position = np.array(p.getBasePositionAndOrientation(body_id2)[0][:2])
                dx= self._entities[entity_id1].position[0] - self._entities[entity_id2].position[0]
                dy= self._entities[entity_id1].position[1] - self._entities[entity_id2].position[1]

                constraint_id = p.createConstraint(body_id1, -1, body_id2, -1, p.JOINT_POINT2POINT,
                                                   [0, 0, 0], [0, 0, 0], [dx, dy, 0])
                ...
                raise ValueError(f"One or both entities ({entity_id1}, {entity_id2}) do not exist in the environment.")
    ...
    def remove_joint(self, entity_id1: int, entity_id2: int):
        ...
        if (entity_id1, entity_id2) in self.joints_map:
            ...
            raise ValueError(f"No joint exists between entities {entity_id1} and {entity_id2}.")
    ...
    def set_position(self, entity_id: int, position: np.ndarray):
        ...
        if entity_id in self.entity_map:
            body_id = self.entity_map[entity_id]["body_id"]
            p.resetBasePositionAndOrientation(body_id, [position[0], position[1], 0], [0, 0, 0, 1])
    ...
    def get_entities_state(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        for entity_data in self.entity_map.values():
            if isinstance(entity_data, dict):  # Only process valid entity data (skip joints)
