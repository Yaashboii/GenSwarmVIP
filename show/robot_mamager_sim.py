...
class SimulationEnvironment:
    def __init__(self, num_agents=5):
        ...
        self.positions = np.random.rand(num_agents, 2) * np.array([SCREEN_WIDTH, SCREEN_HEIGHT])
    ...
    def update_positions(self, dt):
        ...
        # 边界处理
        self.positions = np.clip(self.positions, [-SCREEN_WIDTH, -SCREEN_HEIGHT],
    ...
    def save_trajectories_as_gif(self, gif_filename="trajectories.gif"):
        ...
        for t in range(len(self.trajectories)):
            ...
            for i in range(self.num_agents):
                ...
                circle = plt.Circle((pos[0], pos[1]), ROBOT_RADIUS, color=np.array(ROBOT_COLOR) / 255)
                ax.add_patch(circle)
